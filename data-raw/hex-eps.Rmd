...
## Clipboard image and font setup
...
Custom font must be registered before any print()/ggsave() call uses it, so it
renders correctly regardless of graphics device

```{r}
library (showtext)
font_add (
    "Alien Encounters",
    regular = "Alien-Encounters-Regular.ttf",
    bold = "Alien-Encounters-Bold.ttf",
    italic = "Alien-Encounters-Italic.ttf",
    bolditalic = "Alien-Encounters-Bold-Italic.ttf"
)
showtext_auto ()
```
...
s3 <- sqrt (3) / 2
border <- data.frame (
    x = 1 + c (rep (-s3, 2), 0, rep (s3, 2), 0, -s3),
    y = 1 + c (0.5, -0.5, -1, -0.5, 0.5, 1, 0.5)
)
# aspect ratio for image
asp <- diff (range (border$x)) / diff (range (border$y))
...
hex <- ggplot () +
    ...
    # force output height to exactly equal width, regardless of device shape
...
th <- theme_minimal ()
th$panel.background <- element_rect (fill = "transparent", linewidth = 0)
th$plot.background <- element_rect (fill = "transparent", colour = NA)
th$line <- element_blank ()
th$axis.text <- element_blank ()
th$axis.title <- element_blank ()
th$plot.margin <- margin (0, 0, 0, 0, unit = "null")
...
setEPS ()
postscript (fname)
hex + theme_minimal (base_family = "Alien Encounters") +
    theme (
...
dev.off ()
ggsave (fname, hex, width = 7, height = 7, bg = "transparent")
ggsave ("autotest.png", hex, width = 7, height = 7, bg = "transparent")
```
