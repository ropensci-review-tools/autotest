...
- [x] T009-1: In `DESCRIPTION`, remove `fs` from the `Suggests:` block and add
...
- [x] T009-2: In `R/zzz.R` (12 sites), replace: `list.files()` → `fs::dir_ls(..., fail = FALSE)`
...
- [x] T009-3: In `R/namespace-processing.R` (13 sites), replace all
...
- [x] T009-4: In `R/rd-lookup.R` (6 sites), replace `file.path()` →
...
- [x] T009-5: In `R/utils.R` (4 sites), replace `basename()` →
...
- [x] T009-9: In `tests/testthat/test-statspkg.R`, replace
...
- [x] T009-10: Run the full test suite (`devtools::test()`) and
