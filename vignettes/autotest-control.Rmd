...
`utils::getFromNamespace()` rather than the `:::` notation, since the latter
is generally best avoided outside of interactive use). For example, a single
`.Rd` file often describes the functionality of several functions, each of
which is identified by specifying the function name as an `"alias"`. The
aliases for the `"cor.Rd"` file are:
