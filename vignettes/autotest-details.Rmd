...
output: 
    html_document:
        ...
        theme: flatly
vignette: >
  %\VignetteIndexEntry{Some Further Details}
...
`autotest` to already developed packages. Doing so will commonly generate many
results. To avoid being overwhelmed about what one should do to reduce the
number of diagnostic messages produced by autotest, this vignette provides
a bit more detail on what autotest actually does. Knowing this should help you
to better address any diagnostic messages that are issued.

By default the [`autotest_package()`
...
library (autotest)
```
```{r pkg-lod, eval = TRUE, echo = FALSE}
